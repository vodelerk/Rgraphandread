...
###########################################################
###########################################################
#     Ejemplo selecci???n autom???tica de modelos 
###########################################################

# Primero intentaremos con el paquete Leaps
# Este intenta buscar los mejores "j" modelo que emplean "k" n???mero de variables
...
# empleemos unos datos que est??????n en linea que corresponden 
...
# Cuatro variables que se cre???an importantes fueron medidas:
# - edad (AGE), 
# - Peso de la madre al momento de su ???ltimo periodo mestrual (LWT), 
# - Raza (1 = White, 2 = Black, 3 = Other) (RACE),
# - El n???mero de visitas m???dicas de controland realizadas durante 
#     el primer trimestre del emaprazo (FTV)
# Adem??????s se cuenta con la siguiente informaci???n:
...
# un truco para cambiar el nombre de las variables que est???n en mayusculas a min???sculas
...
# chequear la definic???n de las variables
class(lbw$age)
class(lbw$age)    

##########################################################
# la regresi???n empleando solo las cuatro variables de inter???s ser???a
##########################################################
...
layout(matrix(c(1),1,1) 
       ...
       layout(matrix(c(1),1,1) 
              ...
              ##########################################################  
              ##########################################################  
              # Segundo intentaremos con un m???todo pra remover variables que no sean significativas
              # step()
